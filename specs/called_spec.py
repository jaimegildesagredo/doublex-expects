...
with describe('called'):
    ...
    with describe('with_args'):
        with it('should pass if called with positional arg'):
            ...
            expect(self.method).to(have_been_called.with_args(self.arg1))

        with it('should pass if called with multiple positional args'):
            self.method(self.arg1, self.arg2)

            expect(self.method).to(have_been_called.with_args(self.arg1, self.arg2))

        with it('should pass if called with keyword args'):
            self.method(**self.kwargs)

            expect(self.method).to(have_been_called.with_args(**self.kwargs))

        with it('should pass if called with positional and keyword args'):
            self.method(self.arg1, self.arg2, **self.kwargs)

            expect(self.method).to(have_been_called.with_args(self.arg1, self.arg2, **self.kwargs))
        ...
        with context('#negated'):
            with it('should pass if called with different positional arg'):
                ...
                expect(self.method).not_to(have_been_called.with_args(self.arg1, self.arg2))

            with it('should fail if called with args'):
                self.method(self.arg1, **self.kwargs)

                with failure('not to have been called with args {!r} and {!r}'.format((self.arg1,), self.kwargs)):
                    expect(self.method).not_to(have_been_called.with_args(self.arg1, **self.kwargs))
